...
<div id="pending-posts" class="grid gap-2">
    ...
    >
        <span class="text-gray-500">
            <span class="font-bold wrap-anywhere">{{ $pendingPost->title }}</span>&nbsp;<span class="text-sm break-all">–&nbsp;{{ $pendingPost->sourceName }}</span><span class="text-sm break-all" :if="$pendingPost->source->isAggregation"> via {{ $pendingPost->source->name }}</span>
        </span>

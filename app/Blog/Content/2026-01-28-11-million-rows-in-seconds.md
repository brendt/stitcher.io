...
1. process 11 million date strings to extract the year from it; and
...
Of course, now my projector needs some work, because it only knows how to deal with a `{php}PageVisited` event object. That would be an easy refactor:
...
But actually… why? Why not store the event data straight in database columns? The answer, of course, is convenience. This `{:hl-type:stored_events:}` table doesn't just store _one_ type of events. It's meant to store any event you'd like. Each type of event has different fields, so creating dedicated columns for each value wouldn't work. However, in our experiment, we only actually care about the _date_, and _that one particular field_ happens to be tracked as a separate column: `{:hl-type:stored_events:}.{:hl-property:createdAt:}`.

Let's — for a moment — ditch that whole `{php}json_decode()` call and use `{:hl-type:stored_events:}.{:hl-property:createdAt:}` directly. What would happen then?
...
Let me know your thoughts! You can leave them [in the comments](#comments), [on Discord](/discord), or you can [subscribe to my newsletter](/mail). Until next time!
...
So here we have a system that doesn't have `ext-sodium` installed, and only relies on `paragonie/sodium_compat` through an intermediary package (awesome-lib). This is the edge-case that Paragon argues as the reason for not removing a redundant dependency. A dependency — mind you — on code that handles cryptography, written in PHP, with an equivalent available in PHP for almost a decade, maintained by a company who makes an effort to not list any names in public, always commenting from a generic company-branded account.

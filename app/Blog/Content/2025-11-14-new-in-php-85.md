...
While definitely a great feature, it's unfortunate that it doesn't work with cloning readonly properties (which I believe to be a very common use case). [I wrote about the problem here](/blog/readonly-or-private-set).
...
The exception can still be surpressed by using the new `({:hl-type:void:})` cast:
...
Some attributes like `{php}#[Override]` or `{php}#[NoDiscard]` can cause compile-time errors when used incorrectly. For example, when adding `#[NoDiscard]` to a function that returns `void`:
...
Adding the new `{php}#[DelayedTargetValidation]` to this function will supress the compile-time error, and will only trigger it if the attribute is being retrieved via reflection:

...
Note that these kinds of closures must always be explicitly marked as `{php}static`, since they aren't attached to a `{php}$this` scope. They also cannot access variables from the outside scope with `{php}use`.
...
Some built-in attributes (like `{php}#[Override]`) are validated at compile-time rather than at runtime when being called via reflection. The `{php}#[DelayedTargetValidation]` allows you to postpone that validation to a runtime:

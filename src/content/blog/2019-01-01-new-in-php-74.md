...
- Scheduled to be released on [November ((#21#)), ((#2019#))](*https://wiki.php.net/todo/php74)
...
There's lots to say about this feature, so I wrote a [dedicated post](*/blog/typed-properties-in-php-74) about them.
...
## `array_merge` and `array_merge_recursive` without arguments <small>[UPGRADING](*https://github.com/php/php-src/blob/PHP-7.4/UPGRADING#L276)</small>

Since the addition of the spread operator, it can make sense to use `array_merge` like so:

```php
$merged = array_merge(...$arrayOfArrays);
```

To support the edge case when `$arrayOfArrays` would be empty, both `array_merge` and `array_merge_recursive` now allow an empty parameter list.
An empty array will be returned if no arrays were passed.

## `strip_tags` also accepts arrays <small>[UPGRADING](*https://github.com/php/php-src/blob/PHP-7.4/UPGRADING#L259)</small>

Previously it was only possible to strip multiple tags like so:

```php
<hljs prop>strip_tags</hljs>($string, '<a><p>')
```

PHP 7.4 also allows to use an array:

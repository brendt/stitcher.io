...
Exceptions are now allowed to be thrown in `__toString`.
This was previously prohibited because of some old core error handling mechanisms, 
but Nikita pointed out that the "solution" to prohibit exceptions in `__toString` 
didn't actually solve the problem it tried to address.

The RFC is still in voting phase, but with 32 votes for and 0 against, 
it's safe to say it will pass.

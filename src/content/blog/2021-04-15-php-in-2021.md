...
These past years, it's been somewhat a personal goal of mine to spread positive news about PHP. Like I said time and time again: PHP isn't the same language it was ten years ago, and we're very thankful for that. It's a fast and reliable language, used to build large applications at scale. Let's discuss some of the most notable changes within PHP, the language _and_ the community, the past year.
...
Year after year, the core team succeeds in bringing a new stable release to the community, packed with a bunch of features and quality-of-life improvements. The upgrade path also isn't all that difficult anymore. I actually upgraded some of my own projects from PHP 7.4 to PHP 8, and it took only an hour or so per project. There really isn't any good reason to stay behind!

## PHP's type system

There's actually quite a lot of exiting news when it comes to types. [Enums](/blog/php-enums) will be added in [PHP 8.1](/blog/new-in-php-81) and we've also seen some of the maintainers of static analysis tools contributing to PHP's source code by landing their first RFC that adds the [`<hljs type>never</hljs>`](/blog/new-in-php-81#new-never-type-rfc) type.
...
Still no support for generics, unfortunately. There are some major issues in adding them in a dynamically typed language like PHP. Nikita has outlined the problems [here](*https://github.com/PHPGenerics/php-generics-rfc/issues/45). My approach would be the easy way out: only support [runtime-erased generics](*https://www.reddit.com/r/PHP/comments/iuhtgd/ive_proposed_an_approach_to_generics_on_internals/) and rely on static analysis. This approach requires more than a technical solution though, it also requires a mind shift in the broader PHP community. Maybe one day it becomes a viable option, but not as of yet.

## Async PHP

Big news recently: PHP is getting coroutines — aka. green threads — in PHP 8.1! Although, Fibers — that's what they are called — might [not be as big a game-changer as you might think](/blog/fibers-with-a-grain-of-salt).

Even though Fibers themselves might only be a small cog in what is the large async machine, the RFC has spiked interest in the async community again, which is of course great news. Frameworks like [Amphp](*https://amphp.org/) and [ReactPHP](*https://reactphp.org/) are growing in popularity, and recently Laravel [announced](*https://laravel-news.com/laravel-octane) built-in support for [Swoole](*https://www.swoole.co.uk/)

## The community

First of, we should focus on Composer, the de-facto standard package manager. Since October, 2020, it now has a [version 2](*https://blog.packagist.com/composer-2-0-is-now-available/). This version comes with several UX improvements, but most importantly sees extreme performance improvements, sometimes even tripling its speed on clean installs.

Speaking of composer, I like to measure the current state of PHP's ecosystem by looking at the available packages over time. It has seen exponential growth. Last year I spoke about ±25 million downloads a day, today that number has more than doubled and we're looking at ±60 million daily downloads.
...
Let's add with just a big list of great stuff that has been added in PHP these recent years. If you haven't kept up with its development, you really want to check it out. I think it shows the growth of the community and core development team within recent years, and I'm confident there's more to come.
...
All of that to say: PHP is alive and doing extremely well. Every year I feel more and more happy with the direction the language is going, and am looking forward to using it for many years to come! If you're exited as well, you probably want to subscribe to [my newsletter](*/newsletter/subscribe) to stay up-to-date about PHP's development, as well as [follow me on Twitter](*https://twitter.com/brendt_gd). 

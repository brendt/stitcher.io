...
The JIT is enabled by specifying `<hljs prop>opcache.jit_buffer_size</hljs>` in `php.ini`, or by passing it as a `-d` parameter in the PHP CLI:

```php
php -dopcache.jit_buffer_size=100M
``` 

PHP 8 will be released on [November 26, 2020](*https://wiki.php.net/todo/php80). It's a new major version, which means that it will introduce some breaking changes, as well as lots of new features and performance improvements. Right now PHP 8 is in feature freeze, meaning no new features can be added anymore. 
...
Also note that the attribute syntax can still change, it's still [undecided](*https://wiki.php.net/rfc/shorter_attribute_syntax_change#voting).
...
The `<hljs type>Stringable</hljs>` interface can be used to type hint anything that is a string or implements `<hljs prop>__toString</hljs>()`. Furthermore, whenever a class implements `<hljs prop>__toString</hljs>()`, it automatically implements the interface behind the scenes and there's no need to manually implement it.

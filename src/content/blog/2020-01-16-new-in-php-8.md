PHP 8 will be released today, on [November 26, 2020](*https://wiki.php.net/todo/php80). It's a new major version, which means that it will introduce some breaking changes, as well as lots of new features and performance improvements. Right now PHP 8 is in feature freeze, meaning no new features can be added anymore. 
...
If the type is a single type, `<hljs type>ReflectionParameter</hljs>::<hljs prop>getType</hljs>()` returns an instance of `<hljs type>ReflectionNamedType</hljs>`, which you can get its name from:
